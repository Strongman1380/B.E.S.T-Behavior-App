      ...
      return `
        <div style="page-break-after: always; padding: 20px; font-family: Arial, sans-serif;">
          ...
          <div style="margin-bottom: 30px;">
            <strong>Teacher's Name:</strong> ${studentEval.teacher_name || settings?.teacher_name || ''}
            <span style="margin-left: 40px;"><strong>School:</strong> ${studentEval.school || settings?.school_name || ''}</span>
          </div>
          
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
            ${Object.entries(studentEval.time_slots || {}).map(([time, data]) => {
              if (time === '2:30') {
                return `
                  <tr>
